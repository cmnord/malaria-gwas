...
def add_SNP_names(df,SNPs):
    ...
    for i in range(len(df['SNP Start'])):
        names.append(SNPs['SNP'][[int(x) for x in SNPs['Position'].values].index(df['SNP Start'][i])])
...
def filter_cell_types(meta):
    ...
    filtered_meta.to_csv("./data/roadmap_metadata_[filtered].tsv", "\t", index=False)
...
def plot_enriched_snps(x_vals, y_vals, colors):
    ...
    #plt.show()
    plt.savefig('./graph_results/All_SNPs_Within_Enriched_Epigenomic_Regions.png')
...
def plot_malaria_cell_types(primary, background, blood, blood_back, meta):
    ...
    x_vals = sorted(counts1.keys(), key=get_num) # sort by epigenome name (ie: 'E003')
    #x_vals.sort(key=lambda cell_type: mix[cell_type]) # sort by number of snps
    x_vals.sort(key=lambda cell_type: (['Erythrocytes (RBC)'] + list(meta['GROUP'].values))[meta_cell_types.index(cell_type)]) # sort by group name
    y_vals = [mix[key] for key in x_vals]
    plt.figure(figsize=(15,20))

    plt.barh(list(reversed(range(len(y_vals)))), y_vals, color=['blue' if x == 'E066' else 'red' if x == "E000" else meta['COLOR'][meta_cell_types.index(x)] for x in x_vals], tick_label=x_vals)
    ...
    plt.legend(handles=[mpatches.Patch(color='red', label='Erythrocytes (RBC)'), mpatches.Patch(color='blue', label='Liver')] + [mpatches.Patch(color=key, label=colors[key]) for key in colors.keys()], loc=1)
    ...
    #plt.show()
    plt.savefig('./graph_results/All_Cell_Types_With_Enriched_Malaria_SNPs_[normalized].png')

def main():
    primary = create_df("./data/overlaps/malaria_acetylation_overlap.tsv",['Cell Type', 'Chromosome', 'SNP Start', 'SNP End', 'SNP Name'])
    back = create_df("./data/overlaps/background_acetylation_overlap.tsv",['Cell Type', 'Chromosome', 'SNP Start', 'SNP End', 'SNP Name'])
    p_meta = create_df("./data/roadmap_metadata.tsv")

    blood = create_df("./data/overlaps/malaria_rbc_expression_overlap.tsv",['Chromosome', 'SNP Start', 'SNP End', 'SNP Name'])
